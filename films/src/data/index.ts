...
async function createFilm(body: any[]) {
  const film = await axios.post(`http://localhost:8004/film/`, body);
  return film.data;
}

async function deleteFilm(id: string) {
  const film = await axios.delete(`http://localhost:8004/film/${id}`);
  return film.data;
}

async function updateFilm(id: string, body: any[]) {
  const film = await axios.put(`http://localhost:8004/film/${id}`, body);
  return film.data;
}
